...
export default function DatePicker(props: DatePickerProps) {
  ...
  const isSelected = (day: number) => {
    ...
    if (!s) return false;
    return (
      s.getFullYear() === currentMonth().getFullYear() &&
      s.getMonth() === currentMonth().getMonth() &&
      s.getDate() === day
    );
  };
  ...
  return (
    <div class={`relative ${props.class ?? ""}`}>
      <button
        type="button"
        onClick={() => setOpen((o) => !o)}
        class="flex w-full items-center justify-between rounded-[var(--radius-sm)] border border-[var(--border)] bg-[var(--bg-card)] px-3 py-2 text-sm text-left transition-colors hover:border-[var(--accent)]"
      >
        <span
          class={
            selected()
      ...
      <Show when={open()}>
        <div class="absolute z-50 mt-1 rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--bg-card)] p-3 shadow-[var(--shadow-elevation-3)]">
          <div class="flex items-center justify-between mb-2">
            <button
              onClick={prevMonth}
              class="rounded-[var(--radius-sm)] p-1 hover:bg-[var(--bg-secondary)]"
            >
              ‹
            </button>
          ...
          <div class="grid grid-cols-7 gap-0.5 text-center text-xs">
            <For each={["Su", "Mo", "Tu", "We", "Th", "Fr", "Sa"]}>
              {(d) => (
                <div class="py-1 text-[var(--text-secondary)] font-medium">
                  {d}
                </div>
              )}
            </For>
