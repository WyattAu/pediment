...
function BuggyComponent() {
  ...
  if (shouldError())
    throw new Error("Component crashed! This is a simulated error.");
  return (
    <div class="space-y-3">
      <p class="text-sm text-[var(--text-primary)]">
        I am working correctly. Click the button below to simulate a crash.
      </p>
      <button
  ...
  return (
    <div class="rounded-[var(--radius-md)] border-2 border-dashed border-yellow-500/40 bg-yellow-500/5 p-6">
      ...
      <p class="text-xs text-[var(--text-secondary)] mb-4">
        This is a custom error boundary fallback with its own design.
      </p>
      <button
        onClick={props.reset}
    ...
    <div class="space-y-6">
      <section>
        ...
        <div class="rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--bg-secondary)] p-4">
          ...
          >
            <Suspense
              fallback={
                <div class="animate-pulse text-sm text-[var(--text-secondary)]">
                  Loading...
                </div>
              }
            >
      ...
      <section>
        ...
        <div class="rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--bg-secondary)] p-4">
          <ErrorBoundary
            fallback={(err, reset) => (
              <CustomFallback error={err} reset={reset} />
            )}
          >
            <Suspense
              fallback={
      ...
      <section>
        ...
        <p class="text-xs text-[var(--text-secondary)] mb-2">
          Changing the key forces a full remount, clearing the error state.
        </p>
        <div class="rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--bg-secondary)] p-4">
          <ErrorBoundary
            fallback={(err, reset) => (
            ...
            <ErrorBoundary
              fallback={(err, reset) => (
                <div class="rounded border border-red-500/20 bg-red-500/5 p-3">
                  ...
                  <button
                    onClick={reset}
                    class="mt-1 text-xs text-red-300 underline"
                  >
                    Reset
                  </button>
                </div>
